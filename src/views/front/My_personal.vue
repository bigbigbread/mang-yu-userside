	...
	export default {
		...
		methods: {
						...
						if (JSON.stringify(this.file) == '{}') {
							this.request.put("/user", {
								...
								if (res.code === 200) {
									...
									this.student_user = JSON.parse(localStorage.getItem("student_user")) //重新赋值一次，让user值更新
									// 触发父级更新User的方法
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.request.put("/authentication", {
							number: this.student_user.sid,
			...
			queryAuthentication(page, size) {
				this.request.get("/authentication", {
					params: {
						page: page,
						size: size
					},
