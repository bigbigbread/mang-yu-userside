<template>
	<div>
		<div v-if="student_user.length==0" style="margin-top: 60px;">
			...
			<div style="margin: 0 auto;width: 90%;padding-top: 20px;">
				<el-form label-width="80px" size="medium" :model="form" ref="form">
					...
					<el-form-item label="商品描述" prop="description">
						<el-input type="textarea" v-model="form.description" maxlength="120" placeholder="如:XXXX"
							:autosize="{ minRows: 3, maxRows: 6 }" show-word-limit></el-input>
					</el-form-item>
					<el-form-item v-if="goods_list.status=='0'" label="下架商品" prop="status">
						<el-switch v-model="form.status" active-value="3" inactive-value="0" active-color="#13ce66"
							inactive-color="#ccc"></el-switch>
					</el-form-item>
					<el-form-item v-if="goods_list.status=='1'" label="下架商品" prop="status">
						<el-switch v-model="form.status" active-value="3" inactive-value="1" active-color="#13ce66"
							inactive-color="#ccc"></el-switch>
					</el-form-item>
					<el-form-item v-if="goods_list.status=='3'" label="上架商品" prop="status">
						<el-switch v-model="form.status" active-value="0" inactive-value="3" active-color="#13ce66"
							inactive-color="#ccc"></el-switch>
					</el-form-item>
					<div style="font-size: 12px;color: #F56C6C;">
						*更改商品信息需管理员重新审核
					...
					<el-form-item>
						<el-button type="success" @click="save('form')"
							style="width: 100%;height: 50px;border-radius: 25px;">
	...
	export default {
		data() {
			return {
				...
				token: localStorage.getItem("student_user") ? JSON.parse(localStorage.getItem("student_user")):
					null,
				form: {},
						...
						this.$prompt('请输入密码', '验证', {
							...
							if (value == null) {
								...
								this.request.get("/student/login", {
									params: {
										Susername: this.student_user.username,
										Spassword: value,
									}
								}).then(res => {
									if (res.code == '200') {
										...
										this.form.sellusername = this.student_user.username
