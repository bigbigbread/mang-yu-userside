			...
			login() {
				this.$refs['suserForm'].validate((valid) => {
					if (valid) { // 表单校验合法
						...
						//将登录请求的URL改为/user/login，并使用POST方法

							username: this.suser.susername,
							password: this.suser.spassword,
							isAdmin: false
							...
							if (res.code == 200) {
								setTimeout(() => {
									localStorage.setItem("student_user", JSON.stringify(res.data)) // 存储用户信息到浏览器
					...
					if (valid) { // 表单校验合法
						...
						if (this.regist.spassword != this.regist.confirmPassword) {
							...
							this.request.post("/user", {
								...
								//将注册请求的URL改为/user，并使用POST方法


								username: this.regist.susername,
								password: this.regist.spassword
							}).then(res => {
								if (res.code == 200) {
									this.$notify({
										title: '成功',
